            ...
            <table class="min-w-full divide-y divide-gray-200 overflow-x-auto">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="subcat in subcategories" :key="subcat.id">
                        <td class="px-4 py-2 whitespace-nowrap"> {{ subcat.id }} </td>
                        <td class="px-4 py-2 whitespace-nowrap"> {{ subcat.name }}</td>
                        <td class="px-4 py-2 whitespace-nowrap"> {{ subcat.slug }}</td>
                        <td class="px-4 py-2 whitespace-nowrap"> {{ subcat.category }}</td>
                        <td class="px-4 py-2 whitespace-nowrap"><img class="h-10 w-10 rounded" :src="'/'+subcat.image" :alt="subcat.name"> </td>
                        <td class="px-4 py-2 whitespace-nowrap  text-sm font-medium">
                            <router-link :to='{ name: "admin.editsubcategory", params: { id: subcat.id } }' class="text-sm font-medium rounded-md text-white bg-blue-500 p-2 mx-1 text-indigo-600 hover:text-indigo-900"> <i class="pi pi-pencil"></i> </router-link>
                            <button @click="deleteItem(subcat.id)" class="text-sm font-medium rounded-md text-white bg-red-400 p-2 mx-1 text-red-600 hover:text-red-900"> <i class="pi pi-trash"></i> </button>
                        </td>
                    </tr>
                </tbody>
            </table>
...
const fetchSubcategory = async () =>{
  ...
  try {
    let response = await ApiService.get("/api/admin/subcategory", params, headers.value);

    if(response.success){
        subcategories.value = response.data.subcategories.data;
        pagination.value = response.data.subcategories.pagination;
    }else{
...
if (result.isConfirmed) {
    let response = await ApiService.delete(`/api/admin/subcategory/${id}`,headers.value);
...
watch(searchTerm, (newSearchTerm, oldSearchTerm) => {
  if (newSearchTerm !== oldSearchTerm) {
    fetchSubcategory(newSearchTerm);
  }
});
</script>
